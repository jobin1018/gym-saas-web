...
export function CoachQuickLog() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // page used to have). That shortcut trusted ANY session already
      // sitting in this browser as proof THIS token had already been
      // redeemed — including a stale PIN login left open, or a DIFFERENT
      // magic link redeemed earlier in the same browser. Confirmed bug: the
      // same link kept working on a second visit, and a second unrelated
      // link would have silently ridden the first one's session too. Every
      // visit now redeems the token in the URL for real, every time — a
      // bare refresh after a successful redemption now correctly reports
      // "already used" rather than quietly reusing the old session, which
      // is the honest behavior for a genuinely single-use token.
